...
dataList.map((e) => {
    listEl.innerHTML +=
        `
    <li data-num="${e.dial_code}" data-code="${e.code}" class="li_flag">
    <img style="width: 25px;" src="./flags/4x3/${e.code.toLowerCase()}.svg" alt="${e.code}" /> ${e.name} ${e.dial_code}
    </li>
    `
});
...
let liArray = Array.from(document.querySelectorAll(".li_flag"));
liArray.forEach((li) => {
    li.addEventListener(("click"), (e) => {
        ...
        liEl.innerHTML = `
        <li class="li_flag">
    <img style="width: 25px;" src="./flags/4x3/${e.target.dataset.code.toLowerCase()}.svg" alt='${e.target.dataset.code}'/> ${e.target.dataset.num}
    </li>
        `;
    });
});
